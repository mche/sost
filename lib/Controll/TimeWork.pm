...
# { профиль: 1212, объект: 1392, дата: "2017-06-02", _title: "Ломов Стар  пт, 02.06.2017", _dblclick: false, значение: "13" }
  ...
  return $c->render(json=>{error=>"Сохранение не выполнено"})
    if $data->{"значение"} ~~ [qw(Ставка КТУ2 Начислено Суточные/ставка Суточные/сумма Отпускные/ставка Отпускные/сумма Отпускные/начислено Суточные/начислено  РасчетЗП Переработка/ставка Переработка/сумма Переработка/начислено), 'Доп. часы замстрой', 'Доп. часы замстрой/сумма', 'Доп. часы замстрой/начислено'];
