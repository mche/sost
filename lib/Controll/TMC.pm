...
sub сохранить_снаб {# снабжение/закупка и перемещения
  ...
  map {######### позиции
    ...
    if (!$data->{'закупка на складе'} && !$data->{'перемещение'} && $c->model_nomen->это_инструмент($_->{nomen} || $_->{Nomen})) {
