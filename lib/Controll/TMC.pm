...
sub сохранить_снаб {# снабжение/закупка и перемещения
  ...
  map {######### позиции
    ...
    if (!$data->{'перемещение'} && $c->model_nomen->это_инструмент($_->{nomen} || $_->{Nomen})) {
      return $c->render(json=>{error=>"Ошибка закупки позиции инструмента: только через склад! "})
        unless $data->{'$на объект'} && $data->{'$на объект'}{id};
      $tmc->{'наименование'} = [
        $c->model_nomen->полное_наименование($_->{nomen} || $_->{Nomen})
        || @{$tmc->{'наименование'} || []}
        ||  return $c->render(json=>{error=>"Ошибка разбора наименования инструмента позиции"})
      ];
