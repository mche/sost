...
sub save_ask {
  ...
  $c->model_obj->доступ_к_объекту($c->auth_user->{id}, $data->{"объект"}, 384331)->[0]
  ...
  map {######### позиции
    ...
    if (!$data->{'закупка на складе'} && !$data->{'перемещение'} && (!@$n || $n->[0] eq 'инструмент и оборудование')) {#$c->model_nomen->это_инструмент($_->{nomen} || $_->{Nomen})
      return $c->render(json=>{error=>"Ошибка закупки позиции инструмента: только через склад! "})
        unless $data->{'$на объект'} && $data->{'$на объект'}{id};
      #~ 
      $tmc->{'наименование'} = @$n ? $n : $tmc->{'наименование'};
      return $c->render(json=>{error=>"Ошибка закупочного наименования инструмента позиции"})
        unless @{$tmc->{'наименование'}};
    ...
    delete @$tmc{(qw(ts uid количество/принято дата/принято принял списать номенклатура), 'простая поставка')};
...
=cut
    ...
    $data->{'груз'} .= join('〉', @{$pos->{"номенклатура"} || $pos->{"наименование"}}).":\t".$pos->{"количество"}."\n";
  ...
  if (my $id = $data->{'с объекта/id'} || $data->{'$с объекта'} && $data->{'$с объекта'}{id}) {
    $c->model_obj->доступ_к_объекту($c->auth_user->{id}, $id, 384331)->[0]
      or return $c->render(json=>{error=>"Объект недоступен"});
...
sub объекты_список_заявок {
  ...
  $c->model_obj->доступ_к_объекту($c->auth_user->{id}, $obj, 384331)->[0]
    or return $c->render(json=>{error=>"Объект недоступен"});
...
sub список_поставок {# для снабжения закупки и перемещения
  my $c = shift;
  my $param =  shift || $c->req->json || {};
...
sub списки_склад {
  my $c = shift;
  my $param =  $c->req->json || {};
  $param->{'объект'} = 0; # показать кладовщику все закупки и перемещения
  $c->список_поставок($param);
...
sub заявки_с_транспортом {
  ...
  $c->model_obj->доступ_к_объекту($c->auth_user->{id}, $param->{'объект'}{id}, 384331)->[0]
    or return $c->render(json=>{error=>"Объект недоступен"});
...
sub сохранить_поступление {
  ...
  $c->model_obj->доступ_к_объекту($c->auth_user->{id}, $r->{'через базу/id'} || $r->{'объект/id'}, 384331)->[0]
    or return $c->render(json=>{error=>"Объект недоступен"});
...
sub удалить_перемещение {
  ...
  $c->model_obj->доступ_к_объекту($c->auth_user->{id}, $t->{"с объекта/id"}, 384331)->[0]
    or return $c->render(json=>{error=>"Объект недоступен"});
...
sub сохранить_номенклатуру_закупки {# склад
  my $c = shift;
  my $data =  $c->req->json || {};
  ...
  $c->render(json=>{success=>$c->model_nomen->позиция($r->{id})});
