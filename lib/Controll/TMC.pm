...
sub сохранить_снаб {# снабжение/закупка и перемещения
  ...
  map {######### позиции
    ...
    #~ if (!$data->{'перемещение'} && $c->model_nomen->это_инструмент($_->{nomen} || $_->{Nomen})) {
      ...
      #~ $tmc->{'наименование'} = join('〉', $c->model_nomen->полное_наименование($_->{nomen} || $_->{Nomen}))
        #~ or return $c->render(json=>{error=>"Ошибка разбора наименования инструмента позиции"});
...
=cut
    ...
    $data->{'груз'} .= ($pos->{"наименование"} || join('〉', @{$pos->{"номенклатура"}})).":\t".$pos->{"количество"}."\n";
