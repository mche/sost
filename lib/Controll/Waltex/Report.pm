...
sub data {
  ...
  my $o1 = $c->model->остаток_начало('проект'=>$project, 'кошелек'=>$wallet, 'контрагент'=>$контрагент,  'даты'=>$date, );
  
  my $data = !$param->{'место интервалов'} || $param->{'место интервалов'} eq 'столбцы'
    ? $c->_по_столбцам
    : $c->_по_строкам($o1); # вычисляешь накопленное сальдо по интервалам от начального сальдо
  
  $data->{'сальдо'}{'начало'} = $o1;
  $data->{'сальдо'}{'конец'} = $c->model->остаток_конец('проект'=>$project, 'кошелек'=>$wallet, 'контрагент'=>$контрагент,  'даты'=>$date, );
...
sub все_кошельки {# сразу весь список кошельков
  ...
  my $cols = $c->model->итого_колонки();
  my $rows = $c->model->всего_строки_все_кошельки();
  ...
  map {
    ...
    unless ($data->{'строки'}[-1] && $data->{'строки'}[-1]{key} eq  $_->{key}) {# новая строка
      my $o = $c->model->остатки_кошельки_или_контрагенты('кошелек'=>$_->{"кошельки/id"}[0][1], 'даты'=>$date, );# начальное и конеч сальдо по кошельку
      @$r{keys %$o} = values %$o; # остатки на начало и конец
      $r->{'кошелек/id'} = $_->{"кошельки/id"}[0][1];
      push @{$data->{'строки'}}, $r;
    }
    $data->{'строки'}[-1]{'колонки'}{$_->{sign}} = $_->{sum};
  } @$body;
  ...
  $data->{'сальдо'}{'начало'} = $c->model->остаток_начало('проект'=>$project, 'даты'=>$date, );
  $data->{'сальдо'}{'конец'} = $c->model->остаток_конец('проект'=>$project,  'даты'=>$date, );
...
sub все_контрагенты {# сразу весь список контрагентов
  ...
  my $cols = $c->model->итого_колонки();
  my $rows = $c->model->всего_строки_все_контрагенты();
  ...
  map {
    ...
    unless ($data->{'строки'}[-1] && $data->{'строки'}[-1]{key} eq  $_->{key}) {# новая строка
      my $o = $c->model->остатки_кошельки_или_контрагенты('контрагент'=>$_->{"контрагент/id"}, 'даты'=>$date, );# начальное и конеч сальдо по кошельку
      @$r{keys %$o} = values %$o; # остатки на начало и конец
      $r->{'контрагент/id'} = $_->{"контрагент/id"};
      push @{$data->{'строки'}}, $r;
    }
    $data->{'строки'}[-1]{'колонки'}{$_->{sign}} = $_->{sum};
  } @$body;
