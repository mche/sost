...
sub data {
  ...
  my %args = ('проект'=>$project, 'кошелек'=>$кошелек, 'кошелек2'=>$кошелек2, 'объект'=>$объект, $контрагент ? ('контрагент'=>$контрагент) : (), $контрагенты ? ('контрагенты'=>$контрагенты) : (), 'профиль'=>$профиль, 'даты'=>$date, 'интервал'=>\@interval);
...
sub все_контрагенты {# сразу весь список контрагентов/или выботочные ид контрагентов
  ...
  #~ my $профиль = $param->{'профиль'} && $param->{'профиль'}{id};
  $c->model->temp_view_name($c->snapshot_name);
  my %args = ('проект'=>$project, 'даты'=>$date, 'кошелек'=>$кошелек, 'объект'=>$объект, $контрагенты ? ('контрагенты'=>$контрагенты) : ('все контрагенты'=>1),);
  $c->model->снимок_диапазона(\%args);
