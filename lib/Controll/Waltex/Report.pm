...
sub data {
  ...
  $c->model->снимок_диапазона('проект'=>$project, 'кошелек'=>$кошелек, 'объект'=>$объект, 'контрагент'=>$контрагент, 'профиль'=>$профиль, 'даты'=>$date, 'интервал'=>\@interval);
...
sub все_объекты {# сразу весь список объектов
  ...
  #~ my $профиль = $param->{'профиль'} && $param->{'профиль'}{id};
  $c->model->temp_view_name($c->snapshot_name);
  $c->model->снимок_диапазона('проект'=>$project, 'даты'=>$date, 'все объекты'=>1,);
