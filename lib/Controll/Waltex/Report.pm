...
sub data {
  ...
  $c->model->снимок_диапазона('проект'=>$project, 'кошелек'=>$кошелек, 'кошелек2'=>$кошелек2, 'объект'=>$объект, 'контрагент'=>$контрагент, 'профиль'=>$профиль, 'даты'=>$date, 'интервал'=>\@interval);
  
  my $o = $c->model->остатки_период('проект'=>$project,  'даты'=>$date, ) # не отсекать контров и сотр 'контрагент'=>$контрагент, 'профиль'=>$профиль, 
    unless $кошелек || $объект || $контрагент || $профиль; ## общие остатки по конкретным позициям не нужны 'кошелек'=>$кошелек, 'объект'=>$объект,
...
sub все_кошельки {# сразу весь список кошельков
  ...
  $c->model->temp_view_name($c->snapshot_name);
  $c->model->снимок_диапазона('проект'=>$project, 'даты'=>$date, 'кошелек2'=>$кошелек2, 'объект'=>$объект, 'контрагент'=>$контрагент, 'профиль'=>$профиль,);#'все кошельки'=>установлен, потому что не указ кошелек в передаче
...
sub все_кошельки2 {# сразу весь список кошельков перемещения
  ...
  $c->model->temp_view_name($c->snapshot_name);
  $c->model->снимок_диапазона('проект'=>$project, 'даты'=>$date, 'кошелек'=>$кошелек, 'все кошельки2'=>1);#'все кошельки'=>установлен, потому что не указ кошелек в передаче
...
sub все_контрагенты {# сразу весь список контрагентов
  ...
  #~ my $профиль = $param->{'профиль'} && $param->{'профиль'}{id};
  $c->model->temp_view_name($c->snapshot_name);
  $c->model->снимок_диапазона('проект'=>$project, 'даты'=>$date, 'кошелек'=>$кошелек, 'объект'=>$объект, 'все контрагенты'=>1,);
...
sub все_объекты {# сразу весь список объектов
  ...
  my $data = {};
  my $cols = $c->model->итого_колонки();
  $data->{'строки'} = $c->model->всего_остатки_все_объекты('проект'=>$project, 'даты'=>$date,);#всего_строки_все_контрагенты();
