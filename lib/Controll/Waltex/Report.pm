...
sub data {
  ...
  my $o = $c->model->остатки_период('проект'=>$project, 'кошелек'=>$wallet, 'контрагент'=>$контрагент,  'даты'=>$date, );
  #~ $c->app->log->error($c->dumper($o));
  my $data = !$param->{'место интервалов'} || $param->{'место интервалов'} eq 'столбцы'
    ? $c->_по_столбцам
    : $c->_по_строкам($o->{'сальдо1'}); # вычисляешь накопленное сальдо по интервалам от начального сальдо
  
  
  $data->{'сальдо'}{'начало'} = $o->{'сальдо1'};
  $data->{'сальдо'}{'конец'} = $o->{'сальдо2'};#$c->model->остаток_конец('проект'=>$project, 'кошелек'=>$wallet, 'контрагент'=>$контрагент,  'даты'=>$date, );
