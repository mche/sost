...
sub все_кошельки {# сразу весь список кошельков
  ...
  my $data = {};
  my $cols = $c->model->итого_колонки();
  $data->{'строки'} = $c->model->всего_остатки_все_кошельки('проект'=>$project, 'даты'=>$date, 'кошелек2'=>$кошелек2, 'объект'=>$объект, 'контрагент'=>$контрагент, 'профиль'=>$профиль,);#всего_строки_все_кошельки();
...
sub все_контрагенты {# сразу весь список контрагентов
  ...
  my $data = {};
  my $cols = $c->model->итого_колонки();
  $data->{'строки'} = $c->model->всего_остатки_все_контрагенты('проект'=>$project, 'даты'=>$date, 'кошелек'=>$кошелек, 'объект'=>$объект, 'все контрагенты'=>1,);#всего_строки_все_контрагенты();
