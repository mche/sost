...
sub снимок_диапазона {
  ...
  push @union, 'снимок диапазона/union/внутр перемещения'
    unless $param->{'пустое движение'} || $param->{'все объекты'} || $param->{'объект'} || $param->{'контрагент'} || $param->{'все контрагенты'} || $param->{'профиль'} || $param->{'все профили'};
    
  push @union, 'снимок диапазона/union/начисления сотрудникам'
    if !($param->{'пустое движение'} || $param->{'все объекты'} || $param->{'объект'}) && $param->{'профиль'} || $param->{'все профили'};
  
  my @bind = (@{$param->{'интервал'} || [undef, undef]}, @{$param->{'даты'}}, ($param->{'проект'}) x 2,  ($param->{'кошелек'}) x 2,  ($param->{'объект'}) x 2, $param->{'все объекты'}, ($param->{'контрагент'}) x 3, $param->{'все контрагенты'}, ($param->{'профиль'}) x 3, $param->{'все профили'}, $param->{'пустое движение'});
  
  push @bind, (@{$param->{'интервал'} || [undef, undef]}, @{$param->{'даты'}}, ($param->{'проект'}) x 2,  ($param->{'кошелек'}) x 2,                                                                                                 ($param->{'контрагент'}) x 3, $param->{'все контрагенты'}, ($param->{'профиль'}) x 3, $param->{'все профили'},) x scalar @union;
...
sub остатки_период {# общие остатки строка
  ...
  push my @bind, ($param->{'даты'}[0], @{$param->{'даты'}}, ($param->{'проект'}) x 2, ($param->{'кошелек'}) x 2, )
    x (1+ scalar @union);
...
where 
  ...
  and (coalesce(?::int, 0)=0 or "объект/id" is not null) --- все объекты
  ...
  and (?::int is null or ("кошельки/id"[2][2] is null and "контрагент/id" is null and "объект/id" is null and "профиль/id" is null))--- пустое движение
