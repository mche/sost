...
sub unions_bind {
  ...
  push @union, 'снимок диапазона/union/все платежи'
    and (($where{'снимок диапазона/union/все платежи'}, @b) = $self->SqlAb->where({
      ...
      $param->{'кошельки'} ? (' "кошельки/id"[1][2] ' => \[ q| = any(?)|, $param->{'кошельки'} ]) : (),
      $param->{'кошельки/IN'}  ? (' "кошельки/id"[1][2] ' => { -in => \$param->{'кошельки/IN'} },) : (),
      $param->{'пустое движение'}
        ...
        : (( $param->{'все контрагенты'}
          ...
          : ($param->{'контрагент'}
              ...
              : (),
            $param->{'контрагенты'}
              ? (' "контрагент/id" '  => \[ q| = any(?)|, $param->{'контрагенты'} ])
              : (),
          )
        ), ($param->{'все объекты'}
          ? (' "объект/id" ' => { '!=', undef })
  ...
  push @union, 'снимок диапазона/union/внутр перемещения'
    ...
    and push @bind, (@{$param->{'интервал'} || [undef, undef]}, @b) #@{$param->{'даты'}}, ($param->{'проект'}) x 2,  ($param->{'кошелек'}) x 2, $param->{'все кошельки2'}, ($param->{'кошелек2'}) x 2, ($param->{'все кошельки2'}) x 2, $param->{'кошелек2'}, )
      if !($param->{'пустое движение'}  || $param->{'все объекты'} || $param->{'объект'} || $param->{'все контрагенты'} || $param->{'контрагенты'} || $param->{'контрагент'} || $param->{'все профили'} || $param->{'профиль'});# || ($param->{'все кошельки2'} || $param->{'кошелек2'}); || $param->{'объект'}|| $param->{'контрагент'} || $param->{'профиль'}
  ...
  push @union, 'снимок диапазона/union/начисления сотрудникам'
    ...
    and push @bind, (@{$param->{'интервал'} || [undef, undef]}, @b) #@{$param->{'даты'}}, ($param->{'проект'}) x 2,  ($param->{'кошелек'}) x 2, $param->{'все профили'}, ($param->{'профиль'}) x 2, ($param->{'все профили'}) x 2, $param->{'профиль'})
      if !($param->{'пустое движение'} || $param->{'все кошельки'}  || $param->{'кошельки'} || $param->{'кошелек'} || $param->{'кошельки/IN'} || $param->{'все контрагенты'} || $param->{'контрагенты'} || $param->{'контрагент'} || $param->{'все кошельки2'} || $param->{'все объекты'} || $param->{'объект'}  || $param->{'кошелек2'});# || ($param->{'профиль'} || $param->{'все профили'}); || $param->{'объект'} || $param->{'контрагент'} || $param->{'кошелек2'}
  ...
  push @union, 'снимок диапазона/union/начисления сотрудникам/по объектам'
    ...
    and (($where{'снимок диапазона/union/аренда'}, @b) = $self->SqlAb->where({
      ...
      $param->{'до второй даты'} ? (' "дата" ' => { '<' => \[" (?::date + interval '1 days') ", $param->{'даты'}[1]] }) : (' "дата" ' => { -between => \["?::date and ?::date" => @{$param->{'даты'}}],}),
      $param->{'проект'} ? (' "кошельки/id"[1][1] ' => $param->{'проект'}) : (),
      $param->{'контрагент'} ? (' "контрагент/id" ' => $param->{'контрагент'}) : (),
      $param->{'контрагенты'} ? (' "контрагент/id" '  => \[ q| = any(?)|, $param->{'контрагенты'} ]) : (),
    }))
    and push @bind, (@{$param->{'интервал'} || [undef, undef]}, @b) #@{$param->{'даты'}}, ($param->{'проект'}) x 2,  ($param->{'кошелек'}) x 2, $param->{'все профили'}, ($param->{'профиль'}) x 2, ($param->{'все профили'}) x 2, $param->{'профиль'})
      if $param->{'аренда'} || $param->{'все контрагенты'} || $param->{'контрагенты'} || $param->{'контрагент'};
