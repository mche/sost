...
DROP VIEW IF EXISTS "движение ДС/начисления по табелю";
...
select id, ts, "дата", "сумма",
  1::numeric as "sign",
  "категории/родители узла/id"(569::int, true) as "категории",
  "категории/родители узла/title"(569::int, false) as "категория",
  null::text as "контрагент", null::int as "контрагент/id",
  null::int as "кошелек2",
  "профиль", "профиль/id",
  array[["проект", '']]::text[][] as "кошельки", --- проект+кошелек, ...
  array[["проект/id", 0]]::int[][] as "кошельки/id"  --- проект+кошелек, ...
  , "примечание"
from 
