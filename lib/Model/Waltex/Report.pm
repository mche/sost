...
sub unions_bind {
  ...
  push @union, 'снимок диапазона/union/начисления сотрудникам'
    ...
    and push @bind, (@{$param->{'интервал'} || [undef, undef]}, @b) #@{$param->{'даты'}}, ($param->{'проект'}) x 2,  ($param->{'кошелек'}) x 2, $param->{'все профили'}, ($param->{'профиль'}) x 2, ($param->{'все профили'}) x 2, $param->{'профиль'})
      if !($param->{'пустое движение'} || $param->{'все объекты'} || $param->{'все контрагенты'} || $param->{'контрагенты'} || $param->{'контрагент'} || $param->{'все кошельки2'} || $param->{'все объекты'} || $param->{'объект'}  || $param->{'кошелек2'});# || ($param->{'профиль'} || $param->{'все профили'}); || $param->{'объект'} || $param->{'контрагент'} || $param->{'кошелек2'}
