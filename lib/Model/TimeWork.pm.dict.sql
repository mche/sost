...
CREATE OR REPLACE FUNCTION "табель/ставка по объектам"()
RETURNS TABLE("профиль/id" int, "объект/id" int, "дата" date, "табель/id" int, "Ставка" numeric)
AS $func$
BEGIN
--------Ставка по объектам-----------
RETURN QUERY 
select ---t.*, 
  ---og.id=sum."объект" as "этот объект",
  p.id as "профиль/id", ---array_agg(og.id order by t."дата", t.id desc) as "объекты/id",
  og.id  as "объект/id",
  t."дата", t.id,
  text2numeric(t."коммент") as "Ставка"
  ---array_agg(text2numeric(t."коммент") order by  t."дата", t.id desc) as "Ставка"
...
@@ табель/join
  "профили" p
  join refs rp on p.id=rp.id1
  join "табель" t on t.id=rp.id2-- на профили
...
from (
select sum.*,
  text2numeric(k1."КТУ1"[1]) as "_КТУ1",
  text2numeric(k2."КТУ2"[1]) as "_КТУ2",
  ---text2numeric(st1."Ставка"[1]) as "Ставка",
 --- text2numeric(coalesce(sm1."коммент", sm2."коммент")) as "Сумма",
  text2numeric(sm1."Сумма"[1]) as "Сумма",
  text2numeric(pay."Начислено"[1]) as "Начислено",
  text2numeric(dop."Доп. часы замстрой"[1]) as "Доп. часы замстрой",
  text2numeric(dop_sum."Доп. часы замстрой/сумма"[1]) as "Доп. часы замстрой/сумма",
  text2numeric(dop_pay."Доп. часы замстрой/начислено"[1]) as "Доп. часы замстрой/начислено",
  ----day."Суточные",
  ---text2numeric(day_st."коммент") as "Суточные/ставка",
  ---day."Суточные" as "Суточные/смены",---умнож * sum."всего смен"
  descr."Примечание"[1] as "Примечание",
...
left join  (---lateral
select ---t.*
  p.id as "профиль/id", og.id as "объект/id", array_agg(t."коммент" order by t.id) as "КТУ1"
from 
  {%= $st->dict->render('табель/join') %}
...
--limit 1
) k1 on k1."профиль/id"=sum."профиль" and k1."объект/id"=sum."объект"
--------КТУ2-----------
left join  (---lateral
select --t.*
  p.id as "профиль/id", og.id as "объект/id", array_agg(t."коммент" order by t.id) as "КТУ2"
from 
  {%= $st->dict->render('табель/join') %}
where --p.id=sum."профиль"
  --and og.id=sum."объект" -- объект
...
--order by t."дата" desc, t.ts desc
--limit 1
) k2 on k2."профиль/id"=sum."профиль" and k2."объект/id"=sum."объект"
--------Ставка по этому объекту или другим-----------
/*left join lateral (
select *  from with_st1
where "профиль/id"=sum."профиль"
order by "объект/id"=sum."объект" desc---, t."дата" desc, t.ts desc
limit 1
) st1 on true*/
...
left join  (---lateral
select --- t.*
  p.id as "профиль/id", og.id as "объект/id", array_agg(t."коммент" order by t.id) as "Сумма"
from 
  {%= $st->dict->render('табель/join') %}
where ---p.id=sum."профиль"
  ---and og.id=sum."объект" -- объект
...
---order by t."дата" desc, t.ts desc
--limit 1
) sm1 on sm1."профиль/id"=sum."профиль" and sm1."объект/id"=sum."объект"
----------------Начислено по этому объекту---------------------
left join  (---lateral
select ---t.*
  p.id as "профиль/id", og.id as "объект/id", array_agg(t."коммент" order by t.id) as "Начислено"
from 
  {%= $st->dict->render('табель/join') %}
where ----p.id=sum."профиль"
  --and og.id=sum."объект" -- объект
...
--order by t."дата" desc
--limit 1
) pay on pay."профиль/id"=sum."профиль" and pay."объект/id"=sum."объект"
-------- Доп. часы замстрой по этому объекту-----------
left join  (---lateral
select --t.*
  p.id as "профиль/id", og.id as "объект/id", array_agg(t."коммент" order by t.id) as "Доп. часы замстрой"
from 
  {%= $st->dict->render('табель/join') %}
where ---p.id=sum."профиль"
  ----and og.id=sum."объект" -- объект
...
---order by t."дата" desc, t.ts desc
---limit 1
) dop on dop."профиль/id"=sum."профиль" and dop."объект/id"=sum."объект"
-------- Доп. часы замстрой/сумма по этому объекту-----------
left join  (---lateral
select ---t.*
  p.id as "профиль/id", og.id as "объект/id", array_agg(t."коммент" order by t.id) as "Доп. часы замстрой/сумма"
from 
  {%= $st->dict->render('табель/join') %}
where--- p.id=sum."профиль"
  ---and og.id=sum."объект" -- объект
...
---order by t."дата" desc, t.ts desc
--limit 1
) dop_sum on dop_sum."профиль/id"=sum."профиль" and dop_sum."объект/id"=sum."объект"
---------------- Доп. часы замстрой/начислено по этому объекту---------------------
left join  (---lateral
select ---t.*
  p.id as "профиль/id", og.id as "объект/id", array_agg(t."коммент" order by t.id) as "Доп. часы замстрой/начислено"
from 
  {%= $st->dict->render('табель/join') %}
where ---p.id=sum."профиль"
  --and og.id=sum."объект" -- объект
...
--order by t."дата" desc
--limit 1
) dop_pay on dop_pay."профиль/id"=sum."профиль" and dop_pay."объект/id"=sum."объект"
----------------Примечание---------------------
left join  (---lateral
select ---t.*
  p.id as "профиль/id", og.id as "объект/id", array_agg(t."коммент" order by t.id) as "Примечание"
from 
  {%= $st->dict->render('табель/join') %}
where--- p.id=sum."профиль"
  ---and og.id=sum."объект" -- объект
...
--order by t."дата" desc
--limit 1
) descr on descr."профиль/id"=sum."профиль" and descr."объект/id"=sum."объект"
...
where r.id1=sum."профиль" --- or (sum."профиль"=r.id2 and p.id=r.id1)
...
from (
  select sum."профиль", sum.names, sum."дата месяц", /* sum."формат месяц",*/ sum."профиль2/id", sum."профиль2", sum."профиль1/id",
    ...
    array_agg(st1."Ставка" order by sum."объект") as "Ставка",
...
--------Ставка по этому объекту или другим-----------
left join lateral (
select "Ставка"  from "табель/ставка по объектам"() st
where st."профиль/id"=sum."профиль"
order by st."объект/id"=sum."объект" desc, "формат месяц2"(st."дата")= sum."дата месяц"/*"формат месяц2"(?::date)**/ desc, st."дата" desc, st."табель/id" desc
limit 1
) st1 on true
----------------Суточные (теперь не по объектам)---------------------
left join lateral (
  ...
  ---limit 1
  --group by p.id---, "формат месяц2"(t."дата")
) day on true ---sum."профиль"=day."профиль/id" ---and sum."дата месяц"=day."дата месяц" and---
----------------Суточные/ставка (теперь не по объектам)---------------------
...
limit 1
  ...
  --limit 1
  --group by p.id---, "формат месяц2"(t."дата")
  ) day_sum on true ---sum."профиль"=day_sum."профиль/id"--- and sum."дата месяц"=day_sum."дата месяц"---
...
----------------Переработка/начислено (не по объектам)---------------------
  ...
  group by sum."профиль", sum.names, sum."дата месяц", day."Суточные", day_st."коммент", day_sum."Суточные/сумма", day_money."Суточные/начислено",  overwork_st."коммент", overwork_sum."Переработка/сумма", overwork_money."Переработка/начислено", /*sum."формат месяц", */ sum."профиль2/id", sum."профиль2", sum."профиль1/id"
) work
