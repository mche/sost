...
sub ask_docx {
  ...
  $r->{docx_out_file} = sprintf("static/tmp/заявка №%s %s.docx", $r->{номер} || '#'.$r->{id}, $r->{перевозчик} || '?перевозчик?', $r->{"маршрут/откуда"}[0][0].' - '.$r->{"маршрут/куда"}[-1][0]);#от $r->{'дата заявки формат'}  
