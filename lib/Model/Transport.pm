...
sub список_заявок_тмц {
  ...
  $self->dbh->selectall_arrayref($self->sth('тмц', select=>$param->{select} || '*', where=>"where t.id=any(?)"), {Slice=>{}}, @bind, );#$param->{async} ? $param->{async} : (),
}
...
sub ask_docx {
  ...
  $r->{коммент} .= "\r\nДокументы отправлять по адресу: 614068, Пермский край, г. Пермь, ул. Попова, д. 23, офис 311, для ИП Останина Г.У."
    if $r->{'посредник/id'} eq 1393 || $r->{'перевозчик/id'} eq 1393;
  ...
  $r->{python} = $self->dict->{'заявка.docx'}->render(#$self->sth('заявка.docx',
    ...
    #~ responsibility=>join(', ', map { " u'$_' " } (q{Перевозчик несет полную материальную ответственность за перевозимый груз и гарантирует полное возмещение прямых и косвенных затрат Заказчика, наступивших вследствие полной или частичной утраты, недостачи, порчи груза, потери товарного вида. Так же срыва погрузки (за не постановку на погрузку-штраф 2000 руб., за опоздание на погрузку-штраф  2000 рублей).}, q{Заказчик своевременно обязуется подготовить провозные документы и груз к отправке, произвести оплату в согласованные сроки. Взаимоотношения и ответственность сторон определяются условиями ГК РФ и Уставом автомобильного транспорта РФ. В случае переадресации или возникновения любых проблем, не предусмотренных договором-заявкой, или внештатных ситуаций, требующих немедленного реагирования.}, q{Перевозчик обязан немедленно известить нас по телефонам 8 (342) 237-53-52, 8(908)-253-01-00. Ссылаясь на предварительную телефонную договоренность, вся информация, полученная сторонами при выполнении настоящего Заказа, является  конфиденциальной и не подлежит разглашению или использованию вне рамок действующего Договора-Заявки. Соблюдение нейтралитета и коммерческой тайны обязательно!}, q{Настоящая заявка имеет силу договора (факсимильная копия действительна).})),
