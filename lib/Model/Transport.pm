...
sub ask_docx {
  ...
  my $director1 = $r->{"директор1"} && $r->{"директор1"}[0]; # в лице перевозчика
  ...
  $r->{python} = $self->dict->{'заявка.docx'}->render(#$self->sth('заявка.docx',
    ...
    contragent1_face_title=>$director1 || ($r->{'посредник'} && $r->{'посредник'}{'реквизиты'} && $r->{'посредник'}{'реквизиты'}{'в лице'}),
    $r->{'посредник/id'} ? () : (
