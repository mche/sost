...
sub ask_docx {
  ...
  $r->{python} = $self->dict->{'заявка.docx'}->render(#$self->sth('заявка.docx',
    ...
    money=>"$r->{сумма} ($r->{'сумма прописью'})$r->{'сумма прописью/коп'}@{[ $r->{'тип стоимости'} eq 1 ? ' за 1(один) час работы' : $r->{'тип стоимости'} eq 1 ? ' за 1(один) километр пробега' : '' ]}, @{[ $r->{'стоимость/с НДС'} ? 'в т.ч. НДС' : 'без НДС' ]}, по оригиналам ОТТН и бух.документов 3-5 б.д.\nДокументы подписать , печать, подпись, расшифровка",
    comment=>$r->{'коммент'} // '',
