...
---  "дата3" timestamp without time zone, --- когда фактически отработана/закрыта заявка
  ...
  "базы" int[] --- доставка на объект через промежуточные базы, пока одна база
/* связи:
...
alter table "транспорт/заявки" add column "базы" int[]; --- доставка на объект через промежуточные базы, пока одна база
...
from "транспорт/заявки" tz
  ...
  ***/
  ...
  from (
   ...
   left join lateral (--- промежуточные базы
    select array_agg(row_to_json(o)) as "базы/json", array_agg(o.id) as "базы/id"
    from refs r
      join "объекты" o on o.id=r.id1
    where r.id2=tz.id
   ) base on true
