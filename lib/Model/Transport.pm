...
sub сохранить_заявку {
  ...
  } qw(заказчик объект транспорт водитель-профиль категория);
  map {# обратная связь
    if ($data->{$_}) {
...
--- не тут id1("контрагенты")->id2("транспорт/заявки") --- перевозчик (внешний транспорт)
id1("контрагенты")->id2("транспорт/заявки") --- получатель/заказчик
--- убрал id1("транспорт/заявки")->id2("контрагенты") ---  
...
select t.*, ----(case when con.id is null then '★' else '' end) || t.title as title2,
  ...
  p.id as "проект/ид", p.title as "проект",
...
select tz.*,
  ...
  tr."перевозчик/id", tr."перевозчик",
...
from "транспорт/заявки" tz
  /*left join (-- перевозчик (в транспорте)
  ...
  */
  ...
  left join (--- транспорт с категорией и перевозчиком
    select tr.*,
