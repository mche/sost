  ...
  map {# прямые связи
    if ($data->{$_}) {
      ...
      $r->{"связь/$_"} = $rr && $rr->{id}
        ? $self->связь_обновить($rr->{id}, $data->{$_}, $r->{id})
        : $self->связь($data->{$_}, $r->{id});
      my $index = indexOf(qw(перевозчик заказчик  посредник грузоотправитель), $_);
      $r->{"контрагенты"}[$index] = $r->{"связь/$_"}{id}
        if defined $index;
    } else {# можно чикать/нет
      $self->связь_удалить(id1=>$prev->{"$_/id"}, id2=>$r->{id});
    }
  } qw(перевозчик заказчик посредник грузоотправитель транспорт водитель-профиль категория);#объект
  map {# обратная связь
    if ($data->{$_}) {
...
sub заявки_контакт4 {
  ...
  $self->dbh->selectall_arrayref($self->sth('заявки/контакты', cont_num=>4), {Slice=>{}}, ([$id]) x 2,);
}
...
sub ask_docx {
  ...
  $r->{docx_out_file} = sprintf("static/tmp/заявка №%s %s.docx", $r->{номер} || '#'.$r->{id}, $r->{перевозчик} || '?перевозчик?', $r->{"маршрут/откуда"}[0][0].' - '.($r->{"_куда"}[-1][0] && $r->{"_куда"}[-1][0] =~ /^#(\d+)/ ? 'Пермь' : $r->{"маршрут/куда"}[-1][0]));#от $r->{'дата заявки формат'}  
  ...
  $r->{python} = $self->dict->{'заявка.docx'}->render(#$self->sth('заявка.docx',
    ...
    director1=>$r->{"директор1"} && $r->{"директор1"}[0],
    bad_director1=>$r->{"директор1"} && $r->{"директор1"}[0] ? '' : '!ЛИЦО ПЕРЕВОЗЧИКА?',
    contact1=>($r->{"контакты"}[0] && $r->{"контакты"}[0][0]) // '',
    phone1=>($r->{"контакты"}[0] && $r->{"контакты"}[0][1]) // '',
    route=>$r->{"маршрут/откуда"}[0][0].' - '.($r->{"_куда"}[-1][0] && $r->{"_куда"}[-1][0] =~ /^#(\d+)/ ? 'г. Пермь' : $r->{"маршрут/куда"}[-1][0]),
    gruz=>$r->{груз},
    contragent4 => $r->{грузоотправитель},
    contact4=>($r->{"контакты"}[3] && $r->{"контакты"}[3][0]) // '',
    phone4=>($r->{"контакты"}[3] && $r->{"контакты"}[3][1]) // '',
    ...
    contact2=>($r->{"контакты"}[1] && $r->{"контакты"}[1][0]) // '',
    phone2=>($r->{"контакты"}[1] && $r->{"контакты"}[1][1]) // '',
