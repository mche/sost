...
sub ask_docx {
  ...
  return $self->sth('заявка.docx',
    ...
    money=>"$r->{сумма} () , без НДС по оригиналам ОТТН и бух.документов 1-3 б.д. Документы подписать , печать, подпись, расшифровка",
    comment=>$r->{коммент},
    contact3=>$r->{"контакт3"}[0],
    phone3=>$r->{"контакт3"}[1],
    transport1=>$r->{"транспорт1"},
