...
sub сохранить_заявку {
  ...
  map {# прямые связи
    ...
    } elsif ($prev->{"$_/id"}) {# можно чикать/нет
      #~ $self->связь_удалить(id1=>$prev->{"$_/id"}, id2=>$r->{id});
...
sub ask_docx {
  ...
  $r->{python} = $self->dict->{'заявка.docx'}->render(#$self->sth('заявка.docx',
    ...
    contact4=>($r->{"контакты грузоотправителя"}[0] && $r->{"контакты грузоотправителя"}[0][0]) // '',
    phone4=>($r->{"контакты грузоотправителя"}[0] && $r->{"контакты грузоотправителя"}[0][1]) // '',
    ...
    contact2=>($r->{"контакты заказчика"}[0] && $r->{"контакты заказчика"}[0][0]) // '',
    phone2=>($r->{"контакты заказчика"}[0] && $r->{"контакты заказчика"}[0][1]) // '',
