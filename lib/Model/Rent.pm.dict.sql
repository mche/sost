...
select
  ...
  coalesce(num1."месяц", now())::date as "дата счета",
  ...
  dp."сумма", dp."сумма"::numeric as "сумма/num",---replace(dp."сумма"::numeric::text, '.', ',') as "сумма/num",
  ...
  join /*lateral*/ (
    ...
    from (
      select
        ...
        /*-1::numeric**/coalesce(nullif({%= $безнал_или_всего || 0 %}*(case when 929979=any(dp."категории")  then   dp."вся сумма" else  dp."сумма" end), 0::money), (case when 929979=any(dp."категории")  then   dp."вся сумма безнал" else  dp."сумма безнал" end)) as "сумма", --- тут нужно всю сумму для обеспечительного платежа
      ...
      from "движение ДС/аренда/счета" dp
        join  (VALUES (1, 'январь'), (2, 'февраль'), (3, 'март'), (4, 'апрель'), (5, 'май'), (6, 'июнь'), (7, 'июль'), (8, 'август'), (9, 'сентябрь'), (10, 'октябрь'), (11, 'ноябрь'), (12, 'декабрь'))  m(num, "месяц") on m.num=date_part('month', dp."дата")
