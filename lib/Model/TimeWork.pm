...
sub сохранить {# из формы и отчета
  ...
  unless ($data->{'значение'} ~~ [qw(Начислено Примечание Суточные/ставка Суточные/сумма Суточные/начислено Отпускные/ставка Отпускные/сумма Отпускные/начислено РасчетЗП Переработка/ставка Переработка/сумма Переработка/начислено)]) {# заблокировать сохранение если Начислено
  ...
  unless ($data->{'значение'} ~~ [qw(Сумма КТУ2 Ставка Начислено Примечание Суточные/ставка Суточные/сумма Суточные/начислено Отпускные/ставка Отпускные/сумма Отпускные/начислено РасчетЗП Переработка/ставка Переработка/сумма Переработка/начислено)]) {
...
where 
  (0::numeric<any(coalesce(sum."Начислено", array[0::numeric]::numeric[])) or coalesce(sum."Отпускные/начислено", 0::numeric)<>0::numeric)
