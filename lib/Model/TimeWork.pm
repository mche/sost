...
sub данные_отчета {
  ...
  #~ if ($param->{'общий список'} || $param->{'объект'}) {
    ...
    return $self->dbh->selectall_arrayref($self->sth('сводка за месяц/общий список', select=>$param->{select} || '*', join_access_obj=>!$access_all_obj, where=>$where, union_double_profiles=>!!$access_all_obj), {Slice=>{},},# join=>'табель/join', 
      ...
      ($param->{'месяц'}) x 11,#6+5 left + 4 full
    );
  #~ }
...
sub квитки_расчет {
  ...
  return $cb
    ? $self->dbh->pg->db->query($self->dict->render('квитки расчет', select=>$param->{select} || '*', where1=>$where1, where=>$where), @bind1, ($param->{'месяц'}) x 12, @bind, $cb)
    ...
    ($param->{'месяц'}) x 12,# параметры для сводка за месяц/общий список (+1 месяц)
    @bind,)
  ;
}
