...
sub данные {# для формы
  ...
  map {
    ...
    elsif ($_->{"значение"} ~~ [qw(КТУ1 КТУ2 КТУ3 Примечание Выплачено _добавлен_)]) {$profiles{$_->{"профиль"}} = {} unless ref $profiles{$_->{"профиль"}}; $profiles{$_->{"профиль"}}{$_->{"значение"}} = $_;} # кту ставит на участке
