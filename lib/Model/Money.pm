...
!!!!!!!!!!!!!!!!!!!!!
Эта таблица использ для зарплатных расчетов квитков ЗП сотрудников
При этом, вместо обязательной связи id1("кошелек")->id2("движение денег") делается
обязательная связь id1("профиль")->id2("движение денег")
Поэтому эти записи не попадут в балансовые расчеты по сотрудникам и движению денег

Однако, когда закрывается зарплатный расчет, то меняется направление связей
id2("движение денег")->id1("профиль") и тогда записи попадают в балансовые расчеты и сводки по деньгам
*/
