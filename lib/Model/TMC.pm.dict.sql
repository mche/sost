  ...
  join (
    ...
    from refs r
      ...
      left join (--- если закупка через базу
        select o.*, r.id as "на объект/id"
        from refs r
          join "roles" o on o.id=r.id1
      
      ) o1 on tz."на объект"=o1."на объект/id" ----=any(array[, tz."с объекта"])
      
      left join (--- если перемещение с объекта
        select o.*, r.id as "с объекта/id"
        from refs r
          join "roles" o on o.id=r.id1
      
      ) o2 on tz."с объекта"=o2."с объекта/id"
  ) tzo on tzo.id1=t.id
  ...
  (
    ...
    from (
      ...
      from
        ...
        left  join inv on d."объект/id"=inv."объект/id" and d."номенклатура/id"=inv."номенклатура/id" and d."дата"<=inv."дата"[1]--- d."дата/принято"
...
{%= $order_by || qq|order by d."дата"::date desc, d."движение"='инвентаризация' desc, d."объект2/id", d.id desc | %} ---- при списании одинаковые строки
;
