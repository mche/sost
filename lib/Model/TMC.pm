...
sub сохранить_снаб {# обработка снабжения + перемещения сохраняются в транспортную заявку
  ...
  map {
    #~ $self->связь_удалить(id1=>$_, id2=>$r->{id})
    $self->_удалить_строку($uid, 'тмц', $_)
      unless $ref{"$_:$r->{id}"};
...
#~ $self->app->log->error($self->app->dumper($r));
  ...
  my $rc = $self->_удалить_строку($uid, 'транспорт/заявки', $data->{id});
  $self->_удалить_строку($uid, 'тмц', $_)
...
sub удалить_инвентаризацию {
  ...
  my $rc = $self->_удалить_строку($uid, 'тмц/инвентаризации', $data->{id});
  $self->_удалить_строку($uid, 'тмц', $_)
    for @{ $prev->{'@позиции тмц/id'} || []};
  return $rc;
}
...
sub удалить_списание {
  ...
  my $rc = $self->_удалить_строку($uid, 'тмц/списания', $data->{id});
  $self->_удалить_строку($uid, 'тмц', $_)
    for @{ $prev->{'@позиции тмц/id'} || []};
  return $rc;
}
...
sub удалить_заявку {
  ...
  return $self->_удалить_строку($uid, 'тмц/заявки', $id);
  #~ return $r;
...
sub сохранить_номенклатуру_закупки {
  ...
  $self->model_nomen->удалить_концы($uid);
  return $n;
}
...
sub сохранить_запрос_резерва_остатка {
  ...
  return $self->_удалить_строку($data->{uid} , 'тмц/резерв', $data->{id})
    if $data->{id};
