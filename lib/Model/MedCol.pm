...
select t."название", s.id, timestamp_to_json(s.ts) as "старт теста",
  count(p.*) as "всего вопросов",
  sum(p."правильный ответ") as "правильных ответов"
from rc
  join "медкол"."сессии" s on rc.parent_id=s.id
  join "медкол"."связи" r on s.id=r.id2
  join "медкол"."названия тестов" t on t.id=r.id1
  
  join lateral (-- которые были в этой сессии
    select p.*, case when p."ответ"=1 then 1::int else 0::int end as "правильный ответ"----, rr.id1
    from "медкол"."связи" rr
