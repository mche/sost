...
div({},
<<END_TEXT
Эта электронная почта используется для рассылки документов и дополнительной информации. Для обратной связи с арендодателем просим Вас отправлять письма на <strong>@{[ $param->{garantia}{реквизиты}{email} || ' емэйл "Гарантия" ' ]}</srtong>. Звонить по телефону <strong>@{[ $param->{garantia}{реквизиты}{'тел 2'}[0]  ]}</strong>.
END_TEXT
),
div({},
