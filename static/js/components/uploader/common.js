...
return {
  "methods": {
    ViewIframe(file){/// из uploader/файлы.js
      ...
      var iframe = {"src": file.src, "height": parseInt(window.innerHeight*0.8), "width": window.innerWidth, "content_type": file.content_type, "timeouts":[]};
      if (vm.iframeFile) {
    ...
    CallbackWaitIframeLoad(){/// из uploader/файлы.js
      ...
      if (vm.iframeFile.timeouts.length > 30) ///30*100 мсек = 3000 сек общее ожидание вызова просмотра
        return console.log("CallbackWaitIframeLoad: нет просмотра по timeouts");
      if (!iframe.contentDocument || iframe.contentDocument.URL != 'about:blank')
        return $('#modal-view-in-iframe', $(vm.$el)).modal('open');
