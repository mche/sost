...
var Component = function  ($scope, $rootScope, $timeout, $http, $element, $q, appRoutes, Util, $ТМЦФорма,  $Контрагенты) {///TMCSnabData
  ...
  $c.InitRow = function(row, index){//строку тмц
    ...
    row.nomen = row.nomen || {selectedItem: {id: row['номенклатура/id'] || (row['$тмц/заявка'] && row['$тмц/заявка']['номенклатура/id']) }, /*newItems: (row['наименование'] || []).map(function(it){ return {title: it}; })*/ };
