...
var Component = function  ($scope, $timeout, $element) {
  ...
  $ctrl.$onInit = function(){
    ...
    $ctrl.ready = true;
    $timeout(function(){
      $('.datepicker', $($element[0])).each(function( index ) {
        //~ console.log("datepicker each ", this);
        $(this).pickadate({// все настройки в файле русификации ru_RU.js
          onSet: function(context){ var s = this.component.item.select; $ctrl.param.values[index]  = [s.year, s.month+1, s.date].join('-'); $ctrl.param['формат'][index] = this.$node.val(); }//
        });//{closeOnSelect: true,}
